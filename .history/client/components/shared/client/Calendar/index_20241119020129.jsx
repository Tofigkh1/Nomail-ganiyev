...
export const Calendar = () => {
  ...
  const [eventsForDay, setEventsForDay] = useState<Eventt[]>([]);
  const [title, setTitle] = useState("");
  const [time, setTime] = useState("");

  const handleAddEvent = () => {
    // Aynı saat ve tarihli etkinlik olup olmadığını kontrol et
    const existingEventIndex = eventsForDay.findIndex(
      (event) => event.time === time
    );

    if (existingEventIndex !== -1) {
      // Aynı saat varsa, eski etkinliği güncelle
      const updatedEvents = [...eventsForDay];
      updatedEvents[existingEventIndex] = { title, time };
      setEventsForDay(updatedEvents);
    } else {
      // Yeni etkinliği ekle
      setEventsForDay([...eventsForDay, { title, time }]);
    }
...
const handleSaveEvent = () => {
  const newEvent = {
    title: eventTitle,
    date: popupDate,
    time: `${startEventTime} - ${EndEventTime}`,
    allDay: allDay,
    reminder: reminder,
