...
export const Calendar = () => {
  ...
  const handleSaveEvent = (newEvent) => {
    setEventsForDay((prevEvents) => {
      // Eğer aynı title ve time özelliklerine sahip bir etkinlik varsa, mevcut olanı güncelle
      const existingEventIndex = prevEvents.findIndex(
        (event) => event.title === newEvent.title && event.time === newEvent.time
      );
  
      if (existingEventIndex !== -1) {
        // Eski etkinliği yeni bilgiyle güncelle
        const updatedEvents = [...prevEvents];
        updatedEvents[existingEventIndex] = newEvent;
        return updatedEvents;
      } else {
        // Aynı etkinlik yoksa yenisini ekle
