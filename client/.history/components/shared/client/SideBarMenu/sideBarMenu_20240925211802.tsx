...
export default function Sidebar({ children }: { children: ReactNode }) {
    ...
    return (
        <aside className="h-screen bg-headerColor">
            <nav
                className={`h-full flex flex-col bg-headerColor border-r border-whiteLight3 transition-all duration-200 ease-in-out ml-3 ${expanded ? "w-[125%] h-[130%] " : "w-[95px]"}`}>
...
export function SidebarItem({icon, text, active, onClick}: SidebarItemProps) {
    ...
    return (
        <li
            ...
            className={`group relative flex items-center py-2 px-3 my-1 font-medium rounded-md cursor-pointer duration-300 ease-in-out transition-colors mt-8
          ${
                active
