...
export default function Sidebar({ children }: { children: ReactNode }) {
    ...
    return (
        <aside className="h-screen bg-headerColor">
            <nav
                className={`h-full flex flex-col bg-headerColor border-r border-whiteLight3 transition-all duration-200 ease-in-out ml-3 ${expanded ? "w-[130%] h-[130%] " : "w-[95px]"}`}>
                <div className="p-4 pb-2 flex justify-between items-center">


                </div>
                <ul className=" w-24 flex-1 px-3">{children}</ul>
            </nav>
        </aside>
    );
}
